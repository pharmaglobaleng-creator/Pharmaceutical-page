...
def description(part: dict[str, str]) -> str:
    ...
    return (
        ...
        f"with {oem}. Compatibility is confirmed during quotation."
    )
...
def build_page(part: dict[str, str], parts: list[dict[str, str]]) -> str:
    slug = part["sku"].lower()
    url = f"{SITE}/parts/{slug}/"
    title = f"{part['part_name']} | Kikusui {part['model']} | {part['sku']} | PharmaGlobalEng"
    meta = description(part)
    image_alt = f"Representative studio visualization of {part['part_name']} matched to Kikusui {part['model']}"
    if has_image(part):
    ...
    related = "".join(
        ...
        f'<span>({escape(candidate["sku"])})</span></li>'
...
<section class="part-hero">{visual}<div class="part-intro"><p class="eyebrow">Independent replacement component</p><h1>{escape(part['part_name'])} for Kikusui {escape(part['model'])}</h1><span class="sku-badge">PharmaGlobalEng SKU: {escape(part['sku'])}</span><div class="store-links"><a href="/parts/kikusui/">View in the Kikusui Parts Store →</a><a href="/parts/">Browse all parts →</a></div><p class="lead">{escape(meta)}</p><div class="compatibility"><strong>OEM number: {escape(oem_display(part))}</strong><br>Make: <strong>Kikusui</strong> · Model: <strong>{escape(part['model'])}</strong></div><div class="part-quote-actions"><button class="btn primary quote-cta" type="button" data-pge-cart-add data-part-sku="{escape(part['sku'])}" data-part-name="{escape(part['part_name'])}" data-part-brand="Kikusui" data-part-model="{escape(part['model'])}" data-part-url="/parts/{slug}/">Add to Quote Cart</button><a class="btn secondary email-part-inquiry" href="{escape(inquiry_url(part))}">Email this part</a></div></div></section></div>
<section class="detail-section"><div class="wrap detail-grid"><div class="detail-box"><h2>Verified part mapping</h2><dl><div><dt>Make</dt><dd>Kikusui</dd></div><div><dt>Model</dt><dd>{escape(part['model'])}</dd></div><div><dt>Part name</dt><dd>{escape(part['part_name'])}</dd></div><div><dt>OEM number</dt><dd>{escape(oem_display(part))}</dd></div><div><dt>Part category</dt><dd>{escape(part['category'])}</dd></div><div><dt>Source page</dt><dd>{escape(part['catalog_page'])}</dd></div></dl></div><div class="detail-box"><h2>Photo and verification status</h2><p>{escape(image_status_note(part))}</p><p><strong>Source verification:</strong> {escape(part['verification'])}.</p><p>The source mapping identifies this exact record. Final dimensional fit, material, finish, and machine configuration are confirmed during quotation; no unsupported specifications are published here.</p></div></div></section>
<section class="detail-section"><div class="wrap"><h2>Find this part by its verified references</h2><p class="section-copy">Search recognizes only the matched part name, Kikusui model, OEM number when supplied, and PharmaGlobalEng SKU.</p><div class="aliases">{alias_chips}</div></div></section>
...
def build_card(part: dict[str, str]) -> str:
    ...
    if has_image(part):
        alt = f"Representative studio visualization of {part['part_name']} matched to Kikusui {part['model']}"
        visual = (
...
def build_catalog(parts: list[dict[str, str]]) -> None:
        ...
        "@graph": [
            {
                ...
                "description": f"{total} independently supplied replacement-part records matched by Kikusui model, part name, and OEM cross-reference.",
            },
            {
                "@type": "BreadcrumbList",
                "itemListElement": [
    ...
    page = f'''<!doctype html><html lang="en-US"><head><script id="pge-analytics" src="/assets/js/pge-analytics.js" defer></script><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Kikusui Tablet Press Replacement Parts by Model and OEM | PharmaGlobalEng</title><meta name="description" content="Browse {total} Kikusui replacement-part records matched to model, part name, and catalog-supplied OEM cross-reference."><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="{SITE}/parts/kikusui/"><link rel="stylesheet" href="/assets/css/pharmaglobaleng.css"><link rel="stylesheet" href="/assets/css/store.css"><link rel="stylesheet" href="/assets/css/parts-quote-cart.css?v=1"><script src="/assets/js/parts-search.js?v=1"></script><script type="application/ld+json">{json.dumps(graph, ensure_ascii=False, separators=(',', ':'))}</script></head><body>
...
<main><section class="store-hero"><div class="wrap"><div class="crumb"><a href="/parts/">Parts Store</a> / Kikusui</div><p class="eyebrow">Verified part mapping</p><h1>Replacement Parts Compatible with Selected Kikusui Equipment</h1><p class="lead">Browse {total} independently supplied replacement-part records matched to the exact Kikusui model, part name, and OEM number in the supplied source. {image_total} records include source-matched studio images; unsupported photographs are intentionally omitted.</p><div class="catalog-search"><label class="visually-hidden" for="part-search">Search Kikusui parts</label><input id="part-search" type="search" placeholder="Search part, model, OEM, or PGE number" autocomplete="off"><label class="visually-hidden" for="model-filter">Filter by Kikusui model</label><select id="model-filter"><option value="">All Kikusui models</option>{filters}</select><span class="search-result-count"><strong id="visible-count">{total}</strong> results</span></div></div></section>
<section class="section" id="catalog"><div class="wrap"><div class="catalog-meta"><span><strong>{total}</strong> mapped records · <strong>{image_total}</strong> verified images</span><span>Independent supplier · compatibility confirmation required</span></div><div class="product-grid" id="product-grid">{''.join(build_card(part) for part in parts)}</div><p id="no-results" class="notice" hidden><strong>No matching part was found.</strong> Try a model, part name, OEM number, or PGE number.</p><div class="notice"><strong>Image accuracy policy:</strong> Records without an exact source photograph do not use a random or substitute product image. Images shown are representative studio visualizations matched to the corresponding source record.</div><div class="notice"><strong>Independent supplier and trademark notice:</strong> PharmaGlobalEng is not affiliated with, authorized by, sponsored by, or endorsed by Kikusui or its trademark owner. Kikusui names, model references, and OEM numbers identify potential equipment compatibility only. All trademarks belong to their respective owners.</div></div></section></main>
...
def update_parts_index() -> None:
    ...
    kikusui_card = '''<a class="machine-card" href="/parts/kikusui/">
            ...
            <h3>Replacement Parts Compatible with Selected Kikusui Equipment</h3><p>Browse independently supplied components matched to the exact Kikusui model and catalog-supplied OEM reference.</p><span class="card-link">Browse compatible parts →</span>
          </a>'''
