...
DATA_PATH = ROOT / "data/fette-first-100.json"
...
def update_parts_index() -> None:
    ...
    page = page.replace("Browse 384 representative visualizations", "Browse 484 representative visualizations")
    ...
    card = '''<a class="machine-card" href="/parts/fette/"><span class="status-pill">100 parts</span><div class="card-mark" aria-hidden="true">FT</div><h3>Replacement Parts Compatible with Selected Fette Equipment</h3><p>Browse independently manufactured components organized by Fette model and catalog-supplied OEM cross-reference.</p><span class="card-link">Browse compatible parts →</span></a>'''
    if card not in page:
...
def write_audit(parts: list[dict]) -> None:
    path = ROOT / "data/fette-first-100-content-audit.csv"
...
def main() -> None:
    ...
    if len(parts) != 100 or len({p["sku"] for p in parts}) != 100:
        raise SystemExit("Fette dataset must contain exactly 100 unique records")
