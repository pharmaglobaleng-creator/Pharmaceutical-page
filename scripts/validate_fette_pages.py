...
parts = json.loads((ROOT / "data/fette-first-100.json").read_text(encoding="utf-8"))
errors: list[str] = []
canonicals: set[str] = set()

if len(parts) != 100:
    errors.append(f"Expected 100 records, found {len(parts)}")
...
if len(re.findall(r'<article class="product-card"', catalog)) != 100:
    errors.append("Fette catalog does not contain exactly 100 cards")
if "Natoli" in catalog:
    errors.append("Natoli identifier leaked into Fette catalog")
if len(re.findall(r'https://pharmaglobaleng\.com/parts/pge-fet-\d{3}/', (ROOT / "sitemap.xml").read_text(encoding="utf-8"))) != 100:
    errors.append("Main sitemap does not contain exactly 100 Fette part URLs")
if len(re.findall(r'https://pharmaglobaleng\.com/parts/pge-fet-\d{3}/', (ROOT / "sitemap-parts-images.xml").read_text(encoding="utf-8"))) != 100:
    errors.append("Image sitemap does not contain exactly 100 Fette part URLs")
