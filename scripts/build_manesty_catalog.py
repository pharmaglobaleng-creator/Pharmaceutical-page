...
def build_page(part: dict[str, str], parts: list[dict[str, str]]) -> str:
    slug = part["sku"].lower()
    url = f"{SITE}/parts/{slug}/"
    title = f"{part['part_name']} | Manesty {part['model']} | {part['sku']} | PharmaGlobalEng"
    meta = description(part)
    image_alt = f"Representative studio visualization of {part['part_name']} matched to Manesty {part['model']}"
    if has_image(part):
...
<section class="detail-section"><div class="wrap detail-grid"><div class="detail-box"><h2>Verified part mapping</h2><dl><div><dt>Make</dt><dd>Manesty</dd></div><div><dt>Model</dt><dd>{escape(part['model'])}</dd></div><div><dt>Part name</dt><dd>{escape(part['part_name'])}</dd></div><div><dt>OEM number</dt><dd>{escape(oem_display(part))}</dd></div><div><dt>Part category</dt><dd>{escape(part['category'])}</dd></div><div><dt>Source page</dt><dd>{escape(part['catalog_page'])}</dd></div></dl></div><div class="detail-box"><h2>Photo and verification status</h2><p>{escape(image_status_note(part))}</p><p><strong>Source verification:</strong> {escape(part['verification'])}.</p><p>The source mapping identifies this exact record. Final dimensional fit, material, finish, and machine configuration are confirmed during quotation; no unsupported specifications are published here.</p></div></div></section>
<section class="detail-section"><div class="wrap"><h2>Find this part by its verified references</h2><p class="section-copy">Search recognizes only the matched part name, Manesty model, OEM number when supplied, and PharmaGlobalEng SKU.</p><div class="aliases">{alias_chips}</div></div></section>
...
def build_card(part: dict[str, str]) -> str:
    ...
    if has_image(part):
        alt = f"Representative studio visualization of {part['part_name']} matched to Manesty {part['model']}"
        visual = (
        ...
        "@graph": [
            {
                ...
                "description": f"{total} independently supplied replacement-part records matched by Manesty model, part name, and OEM cross-reference.",
            },
            {
                "@type": "BreadcrumbList",
                "itemListElement": [
