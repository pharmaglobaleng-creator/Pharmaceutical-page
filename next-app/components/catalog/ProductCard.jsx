...
export default function ProductCard({ part }) {
  ...
  return <article className="pc-card" data-pc-sku={part.sku}>
    ...
    <div className="pc-card-content"><p className="pc-card-family">{part.family}</p><h2><a href={part.url}>{part.name}</a></h2><dl className="pc-identifiers"><div><dt>Model</dt><dd>{part.brandName} {part.model}</dd></div><div><dt>PGE number</dt><dd>{part.sku}</dd></div><div><dt>Replacement OEM Number</dt><dd>{part.oem}</dd></div></dl><div className="pc-card-actions"><a href={part.url} className="pc-button pc-button-secondary">View details <span aria-hidden="true">↗</span></a><QuoteButton part={quotePart} /></div></div>
  </article>;
}
