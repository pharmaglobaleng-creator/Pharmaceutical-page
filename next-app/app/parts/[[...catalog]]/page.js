...
import { SITE, PAGE_SIZE, catalogData, catalogPath, catalogRoutes, findRoute, routeDetails } from '../../../lib/catalog.mjs';
...
function Schema({ route, details }) {
  ...
  if (details.products.length) {
    ...
    graph.push({ '@type': 'ItemList', '@id': SITE + route.url + '#items', name: details.name, numberOfItems: route.total, itemListElement: details.products.map((p, i) => ({ '@type': 'ListItem', position: (route.page - 1) * PAGE_SIZE + i + 1, name: p.name, url: SITE + p.url })) });
  }
